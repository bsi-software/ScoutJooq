...
import ${package}.database.table.CodeTypeEnum;
	...
	public void localeCodeTypeTest() throws TooManyRowsException, DataAccessException, SQLException {
		...
		.forEach(code -> {
			...
			if(text.getKey().equals("nn-NO")) {
				...
				Assert.assertEquals("Unexpected name for code '${symbol_escape}" + code.getId() + ${symbol_escape}"' and locale 'und'",  expectedText, text.getText());
			}
		});
